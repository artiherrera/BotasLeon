import { ContentPage } from "@/components/ContentPage"
import { LocalizedLink as Link } from "@/components/LocalizedLink"
import { Localized } from "@/components/Localized"
import { pageMetadata } from "@/lib/seo"
import { whatsappHref } from "@/lib/whatsapp"

export default function ContactoPage() {
  return (
    <ContentPage
      eyebrow="page.contacto.eyebrow"
      title="page.contacto.title"
      intro="page.contacto.intro"
    >
      <Localized
        es={
          <>
            <h2>Atención a clientes</h2>
            <p>
              <strong>Correo:</strong> contacto@botasleon.com<br />
              <strong>WhatsApp:</strong>{" "}
              <a href={whatsappHref("es")} target="_blank" rel="noopener noreferrer">
                +52 479 303 2457
              </a><br />
              <strong>Horario:</strong> Lunes a viernes 9:00 a 18:00 (CDMX)
            </p>
            <p>
              <a
                href={whatsappHref("es")}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex h-12 items-center gap-2 px-6 bg-[#25D366] text-white! text-sm font-medium no-underline! hover:brightness-95 transition-[filter] duration-[180ms]"
              >
                <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51l-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.71.306 1.263.489 1.694.625.712.227 1.36.195 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                </svg>
                Escríbenos por WhatsApp
              </a>
            </p>
            <p>
              Respondemos correos en menos de 24 horas hábiles. Por WhatsApp
              usualmente más rápido durante horario.
            </p>

            <h2>Pedidos mayoristas y marcas</h2>
            <p>
              Si tienes una tienda física, vendes online, o representas una marca
              que quiere distribuirse con nosotros, escríbenos a:
            </p>
            <p>
              <strong>Correo:</strong> contacto@botasleon.com
            </p>

            <h2>Dirección</h2>
            <p>
              BotasLeón<br />
              Blvd. Hilario Medina 407, 2º piso<br />
              Col. Josefina, 37260 León de los Aldama, Gto., México
            </p>
            <p>
              Te esperamos en nuestra tienda.{" "}
              <Link href="/visitanos">Ver ubicación, horarios y cómo llegar →</Link>
            </p>

            <h2>Síguenos</h2>
            <ul>
              <li>Instagram: @botasleon</li>
              <li>Facebook: /botasleon</li>
              <li>TikTok: @botasleon</li>
            </ul>
          </>
        }
        en={
          <>
            <h2>Customer support</h2>
            <p>
              <strong>Email:</strong> contacto@botasleon.com<br />
              <strong>WhatsApp:</strong>{" "}
              <a href={whatsappHref("en")} target="_blank" rel="noopener noreferrer">
                +52 479 303 2457
              </a><br />
              <strong>Hours:</strong> Monday to Friday, 9:00 a.m. to 6:00 p.m. (Mexico City time)
            </p>
            <p>
              <a
                href={whatsappHref("en")}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex h-12 items-center gap-2 px-6 bg-[#25D366] text-white! text-sm font-medium no-underline! hover:brightness-95 transition-[filter] duration-[180ms]"
              >
                <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51l-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.71.306 1.263.489 1.694.625.712.227 1.36.195 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                </svg>
                Message us on WhatsApp
              </a>
            </p>
            <p>
              We reply to emails within 24 business hours. WhatsApp is usually
              faster during business hours.
            </p>

            <h2>Wholesale orders and brands</h2>
            <p>
              If you run a brick-and-mortar store, sell online, or represent a
              brand that wants to distribute with us, write to us at:
            </p>
            <p>
              <strong>Email:</strong> contacto@botasleon.com
            </p>

            <h2>Address</h2>
            <p>
              BotasLeón<br />
              Blvd. Hilario Medina 407, 2º piso<br />
              Col. Josefina, 37260 León de los Aldama, Gto., México
            </p>
            <p>
              Come see us at our store.{" "}
              <Link href="/visitanos">See location, hours and how to get there →</Link>
            </p>

            <h2>Follow us</h2>
            <ul>
              <li>Instagram: @botasleon</li>
              <li>Facebook: /botasleon</li>
              <li>TikTok: @botasleon</li>
            </ul>
          </>
        }
      />
    </ContentPage>
  )
}

export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }) {
  const { lang } = await params
  return pageMetadata({
    locale: lang,
  path: "/contacto",
  title: "Contacto",
  description:
    "Escríbenos a contacto@botasleon.com — resolvemos talla, envíos y devoluciones.",
  })
}
